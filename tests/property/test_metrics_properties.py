...
class TestMetricsProperties:
    """property tests for metrics computation invariants"""

    # confusion matrix cells must never be negative for any input
    @given(n=num_classes, pairs=pairs)
    def test_confusion_matrix_never_negative(self, n, pairs):
    ...
    @given(n=num_classes, pairs=pairs)
    def test_confusion_matrix_dimensions(self, n, pairs):
        valid = [(a, p) for a, p in pairs if a < n and p < n]
        matrix = confusion_matrix(valid, n)
        assert len(matrix) == n
    ...
    @given(n=num_classes, pairs=pairs)
    def test_accuracy_in_unit_interval(self, n, pairs):
        valid = [(a, p) for a, p in pairs if a < n and p < n]
        matrix = confusion_matrix(valid, n)
        acc = accuracy(matrix)
        assert 0.0 <= acc <= 1.0
    ...
    @given(n=num_classes, pairs=pairs)
    def test_per_class_metrics_in_unit_interval(self, n, pairs):
        valid = [(a, p) for a, p in pairs if a < n and p < n]
        matrix = confusion_matrix(valid, n)
    ...
    def test_normalised_values_in_unit_interval(self, matrix):
    ...
    @given(matrix=square_non_negative_int_matrix())
    def test_non_zero_rows_sum_to_one(self, matrix):
    ...
    @given(matrix=square_non_negative_int_matrix())
    def test_zero_rows_stay_zero(self, matrix):
        for raw_row, norm_row in zip(matrix, normalise_confusion_matrix(matrix), strict=True):
            if sum(raw_row) == 0:
    ...
    @given(matrix=square_non_negative_int_matrix())
    def test_normalised_dimensions_match_input(self, matrix):
    ...
    def test_low_confidence_never_exceeds_total(self, flags, confidence, frame_ms):
        m = RunningMetrics()
        for f in flags:
    ...
    def test_mean_confidence_in_unit_interval(self, flags, confidence):
        m = RunningMetrics()
        for f in flags:
