...
from src.io import build_packet
...
class TestUARTProperties:
    """property tests for build_packet serialisation invariants"""
    ...
    def test_packet_ends_with_terminator(self, fields):
    ...
    def test_packet_separator_count(self, fields):
        pkt = build_packet(fields)
    ...
    @given(st.dictionaries(ascii_key, value_strategy, min_size=0, max_size=10))
    def test_packet_is_ascii(self, fields):
        pkt = build_packet(fields)
        pkt.decode("ascii")

    # serialisation must be deterministic for identical inputs
    @given(st.dictionaries(ascii_key, value_strategy, min_size=0, max_size=10))
    def test_packet_deterministic(self, fields):
