...
def test_all_colour_ids_produce_valid_ndarray(colour_id: ColourID) -> None:
    ...
    out = ov.render(frame, make_preprocess_result(), None, make_decision(label=colour_id), RunningMetrics())
...
def test_uart_connected_any_bool_does_not_crash(uart_connected: bool) -> None:
    ...
    frame = np.zeros((100, 100, 3), dtype=np.uint8)
    out = ov.render(
        frame, make_preprocess_result(), None, make_decision(), RunningMetrics(),
        uart_connected=uart_connected,
    )
    assert out is not None

# output spatial dimensions match input scaled by the configured factor
...
def test_output_shape_matches_scale(scale: float) -> None:
    ...
    with tempfile.TemporaryDirectory() as d:
        ov = Overlay(Config(write_config(data, Path(d))))
    frame = np.zeros((100, 100, 3), dtype=np.uint8)
    out = ov.render(frame, NOT_FOUND, None, None, RunningMetrics())
    assert out is not None
    assert out.shape[:2] == (int(100 * scale), int(100 * scale))
