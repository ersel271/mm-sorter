...
class TestLowSaturationRule:
    ...
    def test_fires_below_threshold(self, default_cfg):
        rule = LowSaturationRule(default_cfg)
    ...
    def test_does_not_fire_at_threshold(self, default_cfg):
        rule = LowSaturationRule(default_cfg)
        threshold = default_cfg.thresholds["sat_min"]
    ...
    def test_confidence_at_zero_saturation(self, default_cfg):
        rule = LowSaturationRule(default_cfg)
        d = rule.apply(make_features(sat_mean=0.0))
        assert d.confidence == pytest.approx(1.0)

    def test_confidence_formula_midpoint(self, default_cfg):
        rule = LowSaturationRule(default_cfg)
        threshold = float(default_cfg.thresholds["sat_min"])
...
class TestHighHighlightRule:
    ...
    def test_fires_above_threshold(self, default_cfg):
        rule = HighHighlightRule(default_cfg)
        d = rule.apply(make_features(highlight_ratio=0.99))
        assert d.label == ColourID.NON_MM

    def test_does_not_fire_at_threshold(self, default_cfg):
        rule = HighHighlightRule(default_cfg)
        threshold = default_cfg.thresholds["highlight_max"]
...
class TestNarrowHuePeakRule:
    ...
    def test_fires_below_threshold(self, default_cfg):
        rule = NarrowHuePeakRule(default_cfg)
        d = rule.apply(make_features(hue_peak_width=1))
        assert d.label == ColourID.NON_MM

    def test_does_not_fire_at_threshold(self, default_cfg):
        rule = NarrowHuePeakRule(default_cfg)
        threshold = default_cfg.thresholds["hue_width_min"]
    ...
    def test_confidence_at_width_one(self, default_cfg):
        rule = NarrowHuePeakRule(default_cfg)
        threshold = float(default_cfg.thresholds["hue_width_min"])
        d = rule.apply(make_features(hue_peak_width=1))
